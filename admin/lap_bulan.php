...
<div class="panel panel-default">
    <div class="panel-heading">
       <strong>Laporan Penjualan Bulanan <small>(<?= tgl_indo($awal) ?> s/d <?= tgl_indo($akhir) ?>)</small></strong>
       <form class="form-inline">
        <input type="hidden" name="m" value="lap_bulan" />
        ...
        <div class="form-group">
            <a class="btn btn-default btn-sm" href="cetak.php?m=lap_bulan&awal=<?= $awal ?>&akhir=<?= $akhir ?>" target="_blank"><span class="glyphicon glyphicon-print"></span> Cetak</a>
        </div>
    </form>
</div>
<div class="panel-body">
    <table id="datatables" class="table table-bordered table-hover table-striped table-condensed" width="100%">
        ...
        $rows = $db->get_results("SELECT MAX(o.tanggal) AS tanggal, p.id_produk, nama_produk, SUM(jumlah) AS jumlah, SUM(jumlah * d.harga) AS total $from WHERE 1 $where GROUP BY YEAR(o.tanggal), MONTH(o.tanggal), p.id_produk LIMIT $offset, $batas");
