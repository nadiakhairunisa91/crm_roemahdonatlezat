...
<div class="panel panel-default">
    <div class="panel-heading">
        <strong>Laporan Penjualan Mingguan <small>(<?= tgl_indo($awal) ?> s/d <?= tgl_indo($akhir) ?>)</small></strong>
        <form class="form-inline">
            <input type="hidden" name="m" value="lap_minggu" />
            ...
            <div class="form-group">
                <a class="btn btn-default btn-sm" href="cetak.php?m=lap_minggu&awal=<?= $awal ?>&akhir=<?= $akhir ?>" target="_blank"><span class="glyphicon glyphicon-print"></span> Cetak</a>
            </div>
        </form>
    </div>
    <div class="panel-body">
        <table id="datatables" class="table table-bordered table-hover table-striped table-condensed" width="100%">
            ...
            $rows = $db->get_results("SELECT MAX(o.tanggal) AS tanggal, WEEK(tanggal) AS minggu, p.id_produk, nama_produk, SUM(jumlah) AS jumlah, SUM(jumlah * d.harga) AS total $from WHERE 1 $where GROUP BY WEEK(o.tanggal), p.id_produk LIMIT $offset, $batas");
            ...
            <tr>
                <td><?= ++$no ?></td>
                <td><?= $periode != $row->minggu ? 'Minggu Per ' . tgl_indo($row->tanggal) : '' ?></td>
                <td><?= $row->nama_produk ?></td>
                <td><?= number_format($row->jumlah) ?></td>
                <td><?= rp($row->total) ?></td>
            </tr>
            <?php $periode = $row->minggu;
            endforeach ?>
        </table>
    </div>
</div>