...
<div class="panel panel-default">
    <div class="panel-heading">
       <strong>Laporan Penjualan produk <small>(<?= tgl_indo($awal) ?> s/d <?= tgl_indo($akhir) ?>)</small></strong>
       <form class="form-inline">
        <input type="hidden" name="m" value="lap_produk" />
        ...
        <div class="form-group">
        <a class="btn btn-default btn-sm" href="cetak.php?m=lap_produk&awal=<?= $awal ?>&akhir=<?= $akhir ?>" target="_blank"><span class="glyphicon glyphicon-print"></span> Cetak</a>
        </div>
    </form>
</div>
<div class="panel-body">
    <table id="datatables" class="table table-bordered table-hover table-striped table-condensed" width="100%">
        ...
        $rows = $db->get_results("SELECT p.id_produk, nama_produk, SUM(jumlah) AS jumlah, SUM(jumlah * d.harga) AS total $from WHERE 1 $where GROUP BY p.id_produk LIMIT $offset, $batas");
