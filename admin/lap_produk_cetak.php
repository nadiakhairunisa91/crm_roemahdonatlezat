...
<h1>Laporan Penjualan produk <small>(<?= tgl_indo($awal) ?> s/d <?= tgl_indo($akhir) ?>)</small></h1>
...
<table id="datatables" class="table table-bordered table-hover table-striped table-condensed" width="100%">
    ...
    $rows = $db->get_results("SELECT p.id_produk, nama_produk, SUM(jumlah) AS jumlah, SUM(jumlah * d.harga) AS total $from WHERE 1 $where GROUP BY p.id_produk");
