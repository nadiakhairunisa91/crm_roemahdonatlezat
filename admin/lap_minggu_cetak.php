...
?>
<h1>Laporan Penjualan Mingguan <small>(<?= tgl_indo($awal) ?> s/d <?= tgl_indo($akhir) ?>)</small></h1>
<table id="datatables" class="table table-bordered table-hover table-striped table-condensed" width="100%">
    ...
    $rows = $db->get_results("SELECT MAX(o.tanggal) AS tanggal, WEEK(tanggal) AS minggu, p.id_produk, nama_produk, SUM(jumlah) AS jumlah, SUM(jumlah * d.harga) AS total $from WHERE 1 $where GROUP BY WEEK(o.tanggal), p.id_produk");
